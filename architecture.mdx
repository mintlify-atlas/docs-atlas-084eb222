...
The path is configurable via `PIMYWA_STATUS` (default: `/tmp/pimywa-status.json`). Because it lives in tmpfs, every write is zero-SD-wear by design.
...
All ports are configurable via environment variables: `PIMYWA_MCP_ADDR`, `PIMYWA_DASH_ADDR` (for the REST API), and the dashboard address. All three are LAN-only by default. For remote access, a WireGuard tunnel is the recommended path.
