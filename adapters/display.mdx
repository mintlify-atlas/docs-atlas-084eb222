---
title: "Display Adapter: E-Paper and File Backends"
sidebarTitle: "Display Adapter"
description: "The Piumy display adapter reads status.json and renders the kaomoji face. Supports three backends: file (PNG, for development), epaper-waveshare, and none."
---
...
| `PIMYWA_ANIM_FAST_SEC` | `2.0` | Fast idle-tick right after a status change |
| `PIMYWA_ANIM_SLOW_SEC` | `18.0` | Slow idle-tick after `PIMYWA_ANIM_RAMP_SEC` of quiet |
| `PIMYWA_ANIM_RAMP_SEC` | `60.0` | Seconds of quiet to ramp FAST → SLOW |
| `PIMYWA_LOW_BATT` | `15` | Battery % below which idle animation is 4x slower |
...
|---|---|---|
| Boot | Full refresh | One-time init: `init()` + `Clear(0xFF)` + base image. Performed by the backend itself before the service loop starts. |
| First content frame | Partial | Drawn from the known-blank boot baseline. |
| Normal animation tick | Partial | No flash; only changed pixels update. |
| Minor mood change | Partial | No flash. |
| Major mood change | Full refresh (opt-in) | Entering or leaving `qr`, `error`, or `sleeping`. Gated by `PIMYWA_EPAPER_FULL_REFRESH` (default `0` / off). |
