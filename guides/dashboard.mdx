---
title: "Using the Piumy Web Dashboard"
sidebarTitle: "Dashboard"
description: "Piumy serves a lightweight web dashboard on LAN port 80. Manage WhatsApp linking, per-chat rules, rate limits, router config, and view battery charts from any browser."
---
...
Set `PIMYWA_DASH` to `off` to disable the dashboard entirely:

```bash
PIMYWA_DASH=off
```

The default is `on`. When disabled, the binary skips starting the dashboard HTTP listener. The REST API (port 8080) and MCP server (port 8081) are unaffected.
